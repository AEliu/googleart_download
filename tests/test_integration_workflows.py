...
class CliIntegrationWorkflowTests(unittest.TestCase):
    ...
    def test_download_artwork_tile_only_returns_visible_tiles_directory(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            hidden_cache_dir = resolve_artwork_cache_dir(
                output_dir,
                page.asset_url,
            ...
            self.assertNotEqual(hidden_cache_dir, visible_tiles_dir)
    ...
    def test_download_artwork_tile_only_rename_reuses_hidden_cache_seeded_from_existing_visible_output(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir)
            visible_tiles_dir = output_dir / "The Great Wave.tiles"
            original_tile_path = visible_tiles_dir / "tiles" / "0-0-0.tile"
            original_tile_path.parent.mkdir(parents=True, exist_ok=True)
            original_tile_path.write_bytes(b"ready-tile")
            (visible_tiles_dir / "state.json").write_text(
                json.dumps(
                    {
                        ...
                        with patch("artx.download.downloader.build_jobs", return_value=jobs):
                            with patch(
                                ...
                                result = download_artwork(
                                    ...
                                    download_size=DownloadSize.MAX,
                                    max_dimension=None,
                                    output_conflict_policy=OutputConflictPolicy.RENAME,
                                    write_metadata=False,
                                    write_sidecar=False,
                                    tile_only=True,
                                    stitch_backend=StitchBackend.AUTO,
            ...
            renamed_tiles_dir = output_dir / "The Great Wave.2.tiles"
            self.assertEqual(result.output_path, renamed_tiles_dir)
            self.assertNotEqual(hidden_cache_dir, renamed_tiles_dir)
            self.assertEqual((renamed_tiles_dir / "tiles" / "0-0-0.tile").read_bytes(), b"ready-tile")
            self.assertEqual(original_tile_path.read_bytes(), b"ready-tile")
